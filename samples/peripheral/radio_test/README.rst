...
The ``data_rate`` command sets the data rate used for any modulated transmission in the Radio Test.
...
.. list-table:: ``fem`` command sub-command list
   ...
   * - Command
     - Description
   * - ``tx_power_control``
     - Set the FEM TX power control to a given value. This value is defined by the FEM in use, and this can only be used when CONFIG_RADIO_TEST_POWER_CONTROL_AUTOMATIC is set to n
   * - ``antenna``
...
The ``set_channel_sequence`` command sets a custom channel sequence for the ``start_tx_with_sleep`` command.
...
.. note::
   ...
   Using the print_channel_sequence is not needed, but it makes verifying the configured channel sequence easier

start_channel
=============

The ``start_channel`` command sets the start channel for the TX and RX sweeps.
The command also configures the transmission channel for both modulated and unmodulated constant carriers.
...
The ``start_duty_cycle_modulated_tx`` command starts a modulated constant carrier with a configurable duty cycle.
The data rate and transmission pattern are controlled with the ``data_rate`` and ``transmit_pattern`` commands, respectively,
while the duty cycle is controlled when invoking the ``start_duty_cycle_modulated_tx`` as a value between 1 and 99 as a percentage.
